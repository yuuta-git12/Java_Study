...
- 暗黙の型変換の優先順位
    ...
    - データサイズ（ビット数）が小さい方から大きい方へ、または整数から浮動小数点数へ向かって優先順位が決まる
- 優先順位が大きい型の変数に優先順位が低い型の値を入れることは可能、逆は不可
    - double型の変数にint型の値を代入することは可能
    - long型の変数にfloat型の値を代入すると**コンパイルエラー**となる
